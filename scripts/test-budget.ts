...
async function testBudget() {
    ...
    try {
        ...
        logs.forEach((l: any) => console.log(`- ${l.userName}: ${l.hours}h on ${l.date}`));

        console.log(`Checking all time logs in DB regardless of status/date...`);
        const allLogs = await TimeLog.find({}).lean();
        console.log(`Total time logs in DB: ${allLogs.length}`);
