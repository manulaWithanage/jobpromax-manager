...
import { Task, KPI, ChartDataPoint, PipelineItem } from "@/types";
...
export default function DashboardPage() {
    ...
    // Placeholder for Pipeline Items (Backend support pending)
    const [pipelineItems] = useState<PipelineItem[]>([
        { id: '101', title: 'Dark Mode Support', estEffort: '5 Days', priority: 'Medium', type: 'Incoming' },
        { id: '102', title: 'Audit Logs for Admins', estEffort: '8 Days', priority: 'High', type: 'Incoming' },
        { id: '201', title: 'AI Chatbot Integration', requester: 'Sales Team', dateAdded: '2024-09-15', type: 'Wishlist' },
    ]);

    useEffect(() => {
        const fetchDashboardData = async () => {
            try {
                const [kpis, burnUp, velocity] = await Promise.all([
                    api.getKPIs(),
                    api.getBurnUp(),
                    api.getVelocity()
                ]);
                if (kpis) setKpiData(kpis);
                if (burnUp) setBurnUpData(burnUp);
